...
if __name__ == "__main__":
    ...
    tasks = ["class"]
    n_classes = len(tasks)
    pth_dir = "../../model/ecgfounder/1_lead_ECGFounder.pth"
    device = torch.device(
        "cuda:{}".format(gpu_id) if torch.cuda.is_available() else "cpu"
    )

    model = ft_1lead_ECGFounder(device, pth_dir, n_classes, linear_prob=False)

    input_tensor = torch.randn(2, 1, 1024).to(
        device
    )  # (n_samples, n_channel, n_length)
    output = model(input_tensor)
    print("Output shape:", output.shape)  # Should be (n_samples, n_classes)
