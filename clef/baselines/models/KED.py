...
class XResNet1d(nn.Sequential):  # block:4, expansion:[3, 4, 23, 3]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        block_szs = [
            int(o * widen) for o in [64, 128, 128, 192] + [32] * (len(layers) - 4)
        ]  # for 2-dim as 768
...
if __name__ == "__main__":
    ...
    ecg_model = xresnet1d101(
        num_classes=12, input_channels=12, kernel_size=5, ps_head=0.5
    ).to(device=device)
    ...
    weight_name = "best_valid_all_increase_with_augment_epoch_3.pt"
    model_path = "../../model/ecgfm-ked/" + weight_name
    checkpoint = torch.load(model_path, map_location="cpu")
    ecg_model_state_dict = checkpoint["ecg_model"]
    ecg_model.load_state_dict(ecg_model_state_dict)

    result = ecg_model(torch.rand((64, 12, 1000)).to(device))  # (64,768,32)
