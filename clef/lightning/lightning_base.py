...
class BaseModelClassifier(L.LightningModule):
    ...
    def _log_detailed_multilabel_metrics(self, logits, targets):
        """Log only AUROC and accuracy for multilabel classification and return metrics dict"""
        ...
        logger.info("\nMultilabel Classification Results (only accuracy & AUROC):")
        logger.info(f"Accuracy: {acc:.4f}")
        logger.info(f"AUROC: {auroc:.4f}")

        return {
            "test_auroc": float(auroc) if not np.isnan(auroc) else float('nan'),
            "test_epoch_acc": float(acc)
        }

    def _log_detailed_binary_metrics(self, logits, targets):
        """Log only AUROC and accuracy for binary classification and return metrics dict"""
        ...
        logger.info("\nBinary Classification Results (only accuracy & AUROC):")
        logger.info(f"Accuracy: {acc:.4f}")
        logger.info(f"AUROC: {auroc:.4f}")
    ...
    def _log_detailed_multiclass_metrics(self, logits, targets):
        """Log only AUROC and accuracy for multiclass classification and return metrics dict"""
        ...
        logger.info("\nMulticlass Classification Results (only accuracy & AUROC):")
        logger.info(f"Accuracy: {acc:.4f}")
        logger.info(f"AUROC: {auroc:.4f}")
