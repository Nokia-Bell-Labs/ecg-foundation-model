...
def load_dataset(path, sampling_rate, release=False):
    if path.split("/")[-3].lower() == "ptbxl":
        ...
        raise ValueError("Unknown dataset path: {}".format(path))

    return X, Y


def load_raw_data_icbeb(df, sampling_rate, path):
    os.makedirs("dataset/ICBEB", exist_ok=True)
    if sampling_rate == 100:
        if os.path.exists(path + "dataset/icbeb/raw100.npy"):
            data = np.load(path + "dataset/icbeb/raw100.npy", allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path + "records100/" + str(f)) for f in tqdm(df.index)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open(path + "dataset/icbeb/raw100.npy", "wb"), protocol=4)
    elif sampling_rate == 500:
        if os.path.exists(path + "dataset/icbeb/raw500.npy"):
            data = np.load(path + "dataset/icbeb/raw500.npy", allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path + "records500/" + str(f)) for f in tqdm(df.index)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open(path + "dataset/icbeb/raw500.npy", "wb"), protocol=4)
    return data
...
def load_raw_data_ptbxl(df, sampling_rate, path):
    ...
    if sampling_rate == 100:
        if os.path.exists("dataset/ptbxl/raw100.npy"):
            data = np.load("dataset/ptbxl/raw100.npy", allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path + f) for f in tqdm(df.filename_lr)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open("dataset/ptbxl/raw100.npy", "wb"), protocol=4)
    elif sampling_rate == 500:
        if os.path.exists("dataset/ptbxl/raw500.npy"):
            data = np.load("dataset/ptbxl/raw500.npy", allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path + f) for f in tqdm(df.filename_hr)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open("dataset/ptbxl/raw500.npy", "wb"), protocol=4)
    return data
